...
class MoleculePDetector(torch.nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # A.1: Nitrogen with 4 single-bond neighbours, no hydrogens.
        substructure_a_1_detector_weights = (
            one_hot_encode(100, 7) + no_hydrogens_check_weights,
            np.ones(100).tolist() + np.zeros(10).tolist(),
            one_hot_encode(4, 0),
            -3
        )

        # B.1: Nitrogen with >= 3 CH3 neighbours.
